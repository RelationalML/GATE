...
expSetting = pd.read_csv('ExpSettings_GAT(E).csv',index_col='expId').fillna('').to_dict()
...
for expID in expIDs:
    ...
    for run in runIDs:#range(numRuns):
        ...
        while(epoch<numEpochs and continueTraining):
            ...
            out,attnCoef,smoothnessMetrics,alphaEntropy = model(data.x, data.edge_index,getSelfAttnCoef= saveAlphas or saveAlphasSummary,
            ...
            model.eval()
            with torch.no_grad():
                out,a,smoothnessMetrics,e = model(data.x, data.edge_index,getSelfAttnCoef=False,getEntropy=False)
